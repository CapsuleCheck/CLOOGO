...
export default function Auth() {
  ...
  return (
    <div className='flex flex-col items-center justify-center min-h-screen px-4 py-12 bg-gradient-to-br from-emerald-50/50 via-white to-slate-50'>
      <Link to='/' className='flex items-center gap-2 mb-8'>
        <div className='flex items-center justify-center w-9 h-9 rounded-xl bg-emerald-600'>
          <MapPin className='w-5 h-5 text-white' />
        </div>
        <span className="font-extrabold text-slate-900 text-2xl font-['Manrope']">
          Cloogo
        </span>
      </Link>

      <div className='w-full max-w-md p-8 bg-white border shadow-xl rounded-3xl shadow-slate-200/60 border-slate-100'>
        {/* Tab switcher */}
        <div className='flex p-1 mb-8 rounded-xl bg-slate-100'>
          <button
        ...
        {error && (
          <div
            ...
            className='flex items-center gap-2 px-4 py-3 mb-6 text-sm text-red-600 border border-red-100 bg-red-50 rounded-xl'
          >
          ...
          >
            ...
            <div>
              ...
              <GooglePlacesAutocomplete
                testId='register-neighborhood-input'
                required
                placeholder='Search city, neighborhood, or address'
                value={registerForm.neighborhood}
                onChange={(neighborhood) =>
                  setRegisterForm((prev) => ({ ...prev, neighborhood }))
                }
                onPlaceSelect={({ neighborhood }) =>
                  setRegisterForm((prev) => ({ ...prev, neighborhood }))
                }
              />
              <p className='text-xs text-slate-400 mt-1.5'>
                Helps runners find errands near you
              </p>
            </div>
            <button
